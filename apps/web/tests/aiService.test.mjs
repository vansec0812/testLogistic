...
test('eDO reported VALID for another container is still rejected against Offer fields', async () => {
  globalThis.fetch = async (_url, init) => {
    ...
    return new Response(JSON.stringify({
      ...
      documentType: 'EDO', matchesRegistration: true, summary: 'eDO hợp lệ.', details: [],
      actualContainerNumber: 'CMAU2197439', actualCarrierCode: 'CMA CGM', actualContainerType: '20GP',
...
test('a Booking PDF renamed as eDO is flagged for Ops even when its fields match', async () => {
  respond({
    status: 'VALID', isLegal: true, hasAnomaly: false, requiresOpsReview: false,
    documentType: 'BOOKING', summary: 'Đọc rõ file.', details: [],
    actualContainerNumber: 'MSKU8421093', actualCarrierCode: 'MSK', actualContainerType: '40HC',
  });
  const result = await ai.verifyEdoWithAI(file, { containerNumber: 'MSKU8421093', carrierCode: 'MSK', containerType: '40HC' });
  assert.equal(result.status, 'ANOMALY');
  assert.equal(result.requiresOpsReview, true);
...
test('eDO read before Offer fields are filled is rechecked when those fields change', async () => {
  respond({
    status: 'VALID', isLegal: true, hasAnomaly: false, requiresOpsReview: false,
    documentType: 'EDO', summary: 'eDO rõ ràng.', details: [],
    actualContainerNumber: 'MSKU 8421093', actualCarrierCode: 'Maersk Line', actualContainerType: '40HQ',
  });
  const pending = await ai.verifyEdoWithAI(file, {});
  assert.equal(pending.status, 'MANUAL_REVIEW');
  const matched = ai.reconcileEdoVerificationResult(pending, { containerNumber: 'MSKU8421093', carrierCode: 'MSK', containerType: '40HC' });
  ...
  const changed = ai.reconcileEdoVerificationResult(pending, { containerNumber: 'MSKU8421093', carrierCode: 'MSK', containerType: '20GP' });
...
test('conflicting eDO verdict or explicit Ops flag never becomes automatic success', async () => {
  respond({ status: 'INVALID', isLegal: true, hasAnomaly: false, requiresOpsReview: false, summary: 'Không hợp lệ.' });
  assert.equal((await ai.verifyEdoWithAI(file)).status, 'INVALID');
  respond({ status: 'VALID', isLegal: true, hasAnomaly: false, requiresOpsReview: true, summary: 'Cần xác minh.' });
  assert.equal((await ai.verifyEdoWithAI(file)).status, 'MANUAL_REVIEW');
});
...
const bookingExpected = { bookingNumber: 'BK-12345', carrierCode: 'MSK', containerType: '40HC', cutOffTime: '2026-10-20T23:59:59' };
const bookingObserved = {
  status: 'VALID', isLegal: true, hasAnomaly: false, requiresOpsReview: false, summary: 'Booking rõ ràng.', details: [],
  documentType: 'BOOKING', actualBookingNumber: 'BK12345', actualCarrierCode: 'Maersk Line',
  actualContainerType: '40HQ', actualCutOffDate: '20/10/2026',
};
...
test('Booking matches normalized booking number, carrier, type and dd/mm/yyyy cut-off', async () => {
  globalThis.fetch = async (_url, init) => {
    ...
    return new Response(JSON.stringify(bookingObserved), { headers: { 'Content-Type': 'application/json' } });
  };
...
for (const [field, value, flagged] of [
  ...
  test(`Booking ${field} changes after scan: both create/edit must become a mismatch`, async () => {
    ...
    const changed = ai.reconcileBookingVerificationResult(observed, { ...bookingExpected, [field]: value });
    assert.equal(changed.status, 'ANOMALY');
    ...
    assert.equal(ai.reconcileBookingVerificationResult(changed, bookingExpected).status, 'VALID');
  });
}
...
test('Booking uploaded before typing is pending; it can match after fields are filled', async () => {
  ...
  assert.equal(pending.status, 'MANUAL_REVIEW');
  assert.equal(ai.reconcileBookingVerificationResult(pending, bookingExpected).status, 'VALID');
});
test('Booking missing identity, document type or cut-off never auto succeeds', async () => {
  for (const field of ['actualBookingNumber', 'actualCarrierCode', 'actualContainerType', 'actualCutOffDate', 'documentType']) {
...
test('Booking provider anomaly remains after all manually entered fields match', async () => {
  respond({ ...bookingObserved, status: 'ANOMALY', hasAnomaly: true, isLegal: false, requiresOpsReview: true, summary: 'Có dấu hiệu chắp vá chứng từ.' });
  const result = await ai.verifyBookingWithAI(file, bookingExpected);
  assert.equal(ai.reconcileBookingVerificationResult(result, bookingExpected).status, 'ANOMALY');
});
test('wrong document and transient error stay in the Booking flow, not eDO copy', async () => {
  ...
  assert.equal((await ai.verifyBookingWithAI(file, bookingExpected)).status, 'ANOMALY');
  respond({ message: 'Dịch vụ AI đang bận.', code: 'AI_PROVIDER_UNAVAILABLE' }, 502);
  const result = await ai.verifyBookingWithAI(file, bookingExpected);
...
test('bare VALID without explicit verdict cannot approve a document', async () => {
  ...
  assert.equal((await ai.verifyBookingWithAI(file, bookingExpected)).status, 'MANUAL_REVIEW');
  respond({ ...bookingObserved, hasAnomaly: undefined });
  assert.equal((await ai.verifyBookingWithAI(file, bookingExpected)).status, 'MANUAL_REVIEW');
});
const photoObserved = {
  status: 'OBSERVED', actualContainerNumber: 'TGBU2415784', actualContainerType: '40HQ',
  actualCarrierCode: 'Maersk Line', actualCondition: 'GOOD',
  actualConditionNotes: 'Vách và cửa nguyên vẹn.', summary: 'Đã đọc ảnh.', requiresOpsReview: false,
};
test('photo OCR check digit is never rewritten to match the registration', async () => {
  respond(photoObserved);
  const result = await ai.verifyContainerPhotosWithAI(photos, { ...expected, containerNumber: 'TGBU2415789', carrierCode: 'MSK' });
  assert.equal(result.status, 'MISMATCH');
...
test('photo provider MATCHED without observed identity/condition is manual review', async () => {
  for (const field of ['actualContainerNumber', 'actualCarrierCode', 'actualContainerType', 'actualCondition', 'actualConditionNotes']) {
    ...
    const result = await ai.verifyContainerPhotosWithAI(photos, { ...expected, containerNumber: 'TGBU2415784', carrierCode: 'MSK' });
    assert.equal(result.status, 'MANUAL_REVIEW', field);
...
test('saved Booking comparison is identical for requester and Ops, including errors', async () => {
  ...
  respond(bookingObserved);
  const result = await ai.verifyBookingWithAI(file, { ...bookingExpected, bookingNumber: 'OTHER-BOOKING' });
  ...
  assert.ok(review.getBookingAiEvidence(saved).some(value => value.includes('OTHER-BOOKING')));
  assert.equal(review.reconcileBookingAiCheck(saved, bookingExpected).isValid, true);
  assert.equal(review.bookingNeedsOpsReview({ status: 'VALID', isValid: true }), true);
});

test('blank or transparent images are not eligible for OCR', () => {
  assert.equal(ai.isBlankImagePixels(new Uint8ClampedArray([255,255,255,255, 255,255,255,255])), true);
  assert.equal(ai.isBlankImagePixels(new Uint8ClampedArray([0,0,0,0, 255,0,0,0])), true);
  assert.equal(ai.isBlankImagePixels(new Uint8ClampedArray([0,0,0,255, 255,255,255,255])), false);
});
