    ...
    <div className="space-y-6">
      ...
      {showAddForm && currentRole === 'ENTERPRISE_A' && (
        <div className="bg-white border border-emerald-200 rounded-2xl p-6 space-y-4 shadow-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              <div>
                ...
                <select
                  ...
                  className="w-full border border-slate-200 rounded-xl px-3.5 py-2.5 outline-none focus:ring-2 focus:ring-emerald-500"
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-slate-200 rounded-xl px-3.5 py-2 outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-slate-200 rounded-xl px-3.5 py-2 outline-none focus:ring-2 focus:ring-emerald-500"
                />
                <p className="text-[11px] text-slate-400 mt-1">Cước vận chuyển + nâng hạ nếu đưa về depot thông thường</p>
              </div>
              <div>
          ...
          <div className="flex gap-2 justify-end pt-2 border-t border-slate-100">
            ...
            {eligibleAssets.length > 0 && (
              <button onClick={handleAddOffer} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow-sm">
                Tạo Offer Nháp
              </button>
            )}
          </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500 outline-none bg-white"
          />
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 space-y-3 bg-white border border-slate-200 rounded-2xl p-8">
          ...
          <h3 className="text-base font-bold text-slate-700">Không có Offer nào</h3>
          <p className="text-xs text-slate-500">{search ? 'Không tìm thấy kết quả.' : 'Chưa có nguồn cung phù hợp bộ lọc.'}</p>
        </div>
      ) : (
        <div className="space-y-4">
          {filtered.map(offer => {
            ...
            return (
              <div
                key={offer.id}
                className="bg-white rounded-2xl border border-slate-200 shadow-sm transition-all hover:shadow-md p-5 space-y-3"
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-mono font-bold text-slate-900 text-sm">{offer.asset.containerNumber}</span>
                        <OfferStatusBadge status={offer.status} size="xs" />
                        <span className="text-slate-500 text-xs">{offer.asset.carrierCode} · {offer.asset.containerType}</span>
                      ...
                      <div className="text-[11px] text-slate-400 mt-1 flex items-center gap-3 flex-wrap">
                        ...
                        <span className="text-emerald-700 font-semibold font-mono">T_A: {formatVnd(offer.baselineDepotCostVnd)}</span>
                        <span>{offer.photoUrls.length}/6 ảnh</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {potentialMatches.length > 0 && (
                    <span className="px-3 py-1 rounded-full text-xs font-bold bg-cyan-50 text-cyan-700 border border-cyan-200 flex items-center gap-1.5 shadow-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {(offer.status === 'DRAFT' || offer.status === 'CHANGES_REQUIRED') && currentRole === 'ENTERPRISE_A' && (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white text-xs font-bold flex items-center gap-1.5 shadow-sm"
                      >
